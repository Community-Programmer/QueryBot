...
from querybot_agent.graph_instructions import graph_instructions


class DataFormatter:
    """Formats data for various visualization types."""
    ...
    def _format_line_data(self, results, question):
        ...
        if len(results[0]) == 2:
            ...
            for item1, item2, item3 in results:
                # Determine which item is the x-value and which is the label
                if isinstance(item2, str) and item2 in labels:
                    x_val, label, y_val = item1, item2, item3
                else:
                    x_val, label, y_val = item2, item1, item3

                if x_val not in x_values:
                    x_values.append(str(x_val))

                if label not in data_by_label:
                    data_by_label[label] = [None] * len(x_values)

                # Ensure all labels have the same length
                for lbl in data_by_label:
                    while len(data_by_label[lbl]) < len(x_values):
                        data_by_label[lbl].append(None)

                # Set the y value for this label and x position
                x_index = x_values.index(str(x_val))
                data_by_label[label][x_index] = float(y_val)
        ...
        if len(results[0]) == 2:
            ...
            for entity in entities:
                ...
                for category in labels:
                    # Find the value for this entity and category
                    value = 0
                    for row in results:
                        if row[0] == entity and row[1] == category:
                            value = float(row[2])
                            break
